...
import { useSupportGroups } from "@/lib/SupportGroups";
...
export default function ChooseCause() {
    ...
    const supportGroups = useSupportGroups(); // Use the hook instead of importing static array
    // Remove the choices state and useEffect - use supportGroups directly
    ...
    return (
        <div className="">
                    ...
                    <div className="flex rounded-[10px] leading-[48px] border-solid border-2 border-transparent">
                        <div className="flex w-full items-center bg-chalk rounded-sm px-3">
                            <div className="relative grow">
                                ...
                                >
                                    {chosenGroup !== null && supportGroups.map((opt) => (
            ...
            {supportGroups[chosenGroup] && <div className="w-full flex sm:flex-row flex-col sm:items-start items-center p-5 gap-4 rounded-lg bg-black bg-opacity-90">
                ...
                <div className="text-white text-sm">
                    <p className="font-semibold sm:text-left text-center">{`${supportGroups[chosenGroup].cardTitle}`}</p>
                    <p className="text-white my-1 mr-2 sm:text-left text-center">{`${supportGroups[chosenGroup].cardMessage}`}</p>
                    <p className="mt-2"><span className="opacity-40">{t('support_groups.banner_link_text')}</span> <Link href={supportGroups[chosenGroup].linkTo} className="text-purple-600 underline">Linktree</Link> </p>
                </div>
            </div>}
        </div>
    );
